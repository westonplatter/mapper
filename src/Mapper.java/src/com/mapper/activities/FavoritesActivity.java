...
public class FavoritesActivity extends PreferenceActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			PreferenceManager.setDefaultValues(this, PREFERENCES_NAME,
					PREFERENCES_MODE, R.xml.settings, false);
			m_pref = PreferenceManager
					.getDefaultSharedPreferences(getApplicationContext());// list
																			// of
																			// favorites
																			// initiated.
			...
			nItems = pref_list.size(); // make number of items same as the items
										// already in the list
			Log.i("INFO", "Num Items");
			Log.d("DEBUG:OnCreate",
					"Number of Favorites stored: " + Integer.toString(nItems));
	...
	public void displayFavorites(int caller) {
		try {
			...
			setListAdapter(new ArrayAdapter<String>(this,
					android.R.layout.simple_list_item_1, pref_list));
			...
			lv.setOnItemClickListener(new OnItemClickListener() {
				public void onItemClick(AdapterView<?> parent, View view,
					...
					YelpResultsResponse yelpResultsResponse = yelpQueryManager
							.search(query);
					int numBusinesses = yelpResultsResponse.getBusinesses()
							.size();

					while (numBusinesses > 0) {
						userSelection = yelpResultsResponse.getBusinesses()
								.get(numBusinesses - 1); // .get(position);
						Log.d("DEBUG: current selection = ",
								userSelection.getName());
						if (userSelection.getName().equals(query)) {
							System.out.println("This is a single Result response");
							Intent myIntent = new Intent(view.getContext(),
									SingleSearchResultView.class);
							myIntent.putExtra("callerId", R.id.save); 
							startActivity(myIntent);
						} else {
							Log.d("DEBUG: Did not find selection = ",
									userSelection.getName() + ":" + query);
						}
						numBusinesses--;
					}
				}
	...
	public void savePreference(String new_pref) {
		...
		if (nItems >= MAX_ITEMS) {
			Log.i("INFO",
					"MAX ITEMs reached, remove some favorites to proceed further");
	...
	public void removePreference(String new_pref) {
		...
		while (it.hasNext()) {
			...
			if (fav_item.equals(new_pref)) {
				SharedPreferences.Editor editor = m_pref.edit();
				editor.remove(m.getKey().toString()); // get the key, so we can
														// remove it
				editor.commit();
				Log.d("DEBUG: REMOVE ", new_pref + ":" + fav_item);
				Log.i("INFO", "Favorite removed");
				nItems--;
				break; // skip this entry from being added to list that need to
						// be stored
			}
		}
	}
	...
	protected Dialog onCreateDialog(int id) {
		...
		case 1:
			...
			builder.setMessage(
					"MAX favorites reached, Do you want to remove some favorites?")
					.setCancelable(false)
					.setPositiveButton("Yes",
							new DialogInterface.OnClickListener() {
								public void onClick(DialogInterface dialog,
										int id) {
									// User want to see list of favorites, to
									// make space
